...
Work goes strictly milestone by milestone (M0 → M12, plan section 7); the next milestone starts only when the current one meets its Definition of Done. The usual request is "implement milestone MN" — the `/milestone` skill (`.claude/skills/milestone/`) holds the procedure: design-note first and user sign-off, then types/data → sim → tests → UI, then verification against the DoD, an ADR and a CHANGELOG entry. **M0 (scaffold), M1 (domain and content), M2 (race core), M3 (race screen) and M4 (race control and strategy) are done; M5 (full weekend and season) is next — and its agreed first task, before qualifying and the calendar, is cutting the lap into segments finer than the three timing sectors ([ADR 005](docs/adr/005-race-control.md), "Решено к M5"): a design-note of its own, a core refactor and a full recalibration.** Each milestone is one commit on its own branch (`m0-scaffold`, `m1-domain`, `m2-race-core`, `m3-race-screen`, `m4-race-control`, each branched from the previous one).
...
3. **Race input** — `buildRaceInput(world, pack, round, seed, { control, commands })`: car performance derived from chassis parts + power unit (`car/performance.ts`), driver attributes, pit crew, the decision profiles of the strategist and of each car's race engineer, the weather timeline (`generateWeather`), and a provisional grid (a one-lap shootout until qualifying lands in M5).
...
5. **Playback** (ADR 004) — the race is computed whole in the worker, then played back: `buildReplay(input, result)` lays out every car's sector crossings, and `frameAt(replay, t)` (pure, `sim/race/replay.ts`) returns the frame at race time `t` — order, gaps, tyres, ERS, fuel, the strategist's current plan, track status, events so far. The frame at the flag equals the classification to the millisecond. Inside a sector a car moves along the lap's speed profile (`race/motion.ts`, from the geometry: corner radius, braking, acceleration), so it slows for corners; the timing-line crossings stay the race's own.
6. **Control** (ADR 005) — the player's side of a race is `RaceInput.control` (which team, and how strategy and radio are delegated) plus `commands`, a log of what was said stamped with race time. A command takes effect at the car's first sector entry at or after its stamp, and the store re-simulates the whole race from the start with the same seed after each one (11 ms), then carries on at the same `timeS` — never a resumed engine. The delegate always decides, even in manual mode: a `call` command only replaces which option was applied, so intercepting one decision does not shift anyone's rng stream. Manual mode is "nobody has answered yet" (`by: 'unanswered'`, the car stays out); the pause lives on screen, not in the simulation. Everything the player's pit wall saw — forecasts, decisions, radio history — comes back in `RaceResult.pitWall` (`null` in batch runs, which pay nothing for it).
7. **Measurement** — `race/analysis.ts` measures calibration metrics from the race output (not from model internals); `scripts/sim-batch.ts` aggregates them. `docs/calibration/M2.md` holds the targets and current numbers.

### Decisions and uncertainty

- **`decide()`** (`sim/decide/decide.ts`, plan 5.19): `(context, competence, intent, rng) => Decision<T>`, where `Decision` carries the choice *and* every option considered with its perceived score and reasons. Systems list options and score them as "share of the best achievable" (0..1, on a domain time scale), then call the shared `chooseByScore()`: noise shrinks with skill, blunders get rarer with consistency. Player delegates and AI rivals use the same function; rivals pass their staff's profile and their team character as intent — there is no separate AI path. First live uses: `race/strategy.ts` (`decideRaceStrategy`, `decidePitCall`) and `race/radio.ts` (`decideRadio` — every car's race engineer, the player's and a rival's, on each lap's timing line).
- **`Estimate<T>`** (`sim/knowledge/estimate.ts`, plan 4.1): the only way uncertainty reaches the player. `measure(truth, precision, rng)` is the one place truth and bias are touched; `observe()` builds an estimate from one measurement, `refine()` fuses in more, and `estimateFromModel()` covers what has no truth to measure yet (a forecast) — by its signature it cannot take one. `basis` holds the observer's unclamped mean/sd — never truth, never bias. Estimates are computed at defined moments and stored in state, never recomputed on render. The AI decides on its own team's estimates, never on the truth.
